...
for theta in np.arange(0,np.pi,np.pi/m/2):
    if theta <= theta1:
        ...
        theta_list.append(theta)
        r_out_list.append(d/2 + H1P*H - 2*rho + np.sqrt(rho**2-P**2/(4*np.pi**2)*(np.pi-theta)**2))
...
#print theta_list
#print r_out_list
r_out_boundary_list = [r + P/10.0 for r in r_out_list]
r_in_list = []
...
for seg in range(segment):
    for i,theta in enumerate(theta_list):
        r_interpolated_list[seg].append((r_out_boundary_list[i]-r_in_list[i])/float(segment-1)*float(seg)+r_in_list[i])
